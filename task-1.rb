...
def work
  ...
  File.foreach('data_100000.txt', chomp: true) do |line|
    if line.start_with?('session'.freeze)
      line.gsub!(/\A(session,|user,)/, '')
      cols = line.split(',')

      browser = cols[2].upcase
      total_sessions += 1
      uniq_browsers << browser
      @user_time << cols[3].to_i
      @user_sessions += 1
